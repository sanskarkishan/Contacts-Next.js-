...
export default async function ContactPage({
  ...
  // Fetch contact by ID
  const res = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/contacts/${id}`);
  const contact = await res.json();

  return (
    <div className="flex">
      <SideMenu route="/contacts" />
      <main className="flex-1 w-full text-white px-6 py-8">
        {contact && (
          <section className="max-w-3xl mx-auto shadow-lg rounded-xl p-6">
            <div className="flex items-center gap-6">
              ...
              <div>
                ...
                <p className="text-sm text-gray-400">Contact ID: {id}</p>
              </div>
            </div>
